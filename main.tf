...
resource "azurerm_virtual_hub_route_table_route" "route1" {
  ...
  next_hop          = azurerm_firewall.hubazfw.id
  depends_on = [ azurerm_virtual_hub_routing_intent.intent1 ]
}
...
resource "azurerm_firewall" "spokeazfw" {
  name                = "Spoke-AzureFirewall"
  location            = azurerm_resource_group.RG.location
  resource_group_name = azurerm_resource_group.RG.name
  sku_name            = "AZFW_VNet"
  sku_tier            = "Premium"
...
#firewall logging
resource "azurerm_monitor_diagnostic_setting" "hubfwlogs"{
  name = "hub-fwlogs-${random_pet.name.id}"
...
resource "azurerm_monitor_diagnostic_setting" "spokefwlogs"{
  name = "spoke-fwlogs-${random_pet.name.id}"
  target_resource_id = azurerm_firewall.spokeazfw.id
  log_analytics_workspace_id = azurerm_log_analytics_workspace.LAW.id
  log_analytics_destination_type = "Dedicated"
